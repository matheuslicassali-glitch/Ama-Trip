...
const Trip = () => {
    ...
    const handleStartFileChange = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => setStartPreview(reader.result);
            reader.readAsDataURL(file);
        }
    };

    const handleEndFileChange = (e) => {
        const file = e.target.files[0];
        if (file) {
            setEndFile(file);
            const reader = new FileReader();
            reader.onloadend = () => setEndPreview(reader.result);
            reader.readAsDataURL(file);
        }
    };
    ...
    const completeStart = async (car, driver, finalLocation) => {
        ...
        if (startFile) {
            try {
                const fileName = `start_${Date.now()}_${startFile.name}`;
                const { error } = await supabase.storage.from('trip-photos').upload(fileName, startFile);
                if (error) throw error;
                const { data } = supabase.storage.from('trip-photos').getPublicUrl(fileName);
                photoUrl = data.publicUrl;
            } catch (error) {
                console.error("Error uploading start photo:", error);
                alert("Erro ao enviar foto. A viagem continuará sem a foto.");
            }
        }
    ...
    const completeEnd = async (finalLocation) => {
        ...
        if (endFile) {
            try {
                const fileName = `end_${Date.now()}_${endFile.name}`;
                const { error } = await supabase.storage.from('trip-photos').upload(fileName, endFile);
                if (error) throw error;
                const { data } = supabase.storage.from('trip-photos').getPublicUrl(fileName);
                photoUrl = data.publicUrl;
            } catch (error) {
                console.error("Error uploading end photo:", error);
                alert("Erro ao enviar foto. A viagem será finalizada sem a foto.");
            }
        }
            ...
            <div className="max-w-2xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <form onSubmit={handleStart} className="glass-morphism p-8 rounded-2xl space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <label className={`block border-2 border-dashed ${startPreview ? 'border-primary bg-primary/10' : 'border-white/10'} rounded-xl p-8 text-center hover:bg-white/5 transition-colors cursor-pointer group relative overflow-hidden`}>
                            {startPreview ? (
                                <div className="text-center">
                                    <img src={startPreview} alt="Preview" className="max-h-32 mx-auto rounded mb-2 shadow-lg" />
                                    <p className="text-xs text-primary font-bold">Imagem selecionada (Toque para alterar)</p>
                                </div>
                            ) : (
                                <>
        ...
        <div className="max-w-2xl mx-auto space-y-6 animate-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleEnd} className="glass-morphism p-8 rounded-2xl space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <label className={`block border-2 border-dashed ${endPreview ? 'border-primary bg-primary/10' : 'border-white/10'} rounded-xl p-8 text-center hover:bg-white/5 transition-colors cursor-pointer group relative overflow-hidden`}>
                        {endPreview ? (
                            <div className="text-center">
                                <img src={endPreview} alt="Preview" className="max-h-32 mx-auto rounded mb-2 shadow-lg" />
                                <p className="text-xs text-primary font-bold">Imagem selecionada (Toque para alterar)</p>
                            </div>
                        ) : (
                            <>
